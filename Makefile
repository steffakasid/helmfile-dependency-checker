.PHONY: help build snapshot release test test-integration lint lint-fix clean generate
...
test:
	go test -v -race -coverprofile=coverage.out $(shell go list ./... | grep -v integration) 2>&1 | tee test-output.log
